  ...
  const tasks = opts.repos.map(
    (full) => pool(async () => {
      ...
      try {
        ...
        const repoTimeoutMs = Math.max(3e4, Number(process.env.REPO_TIMEOUT_MS ?? "180000"));
        if (useGql) {
          try {
      ...
      try {
        ...
        const rawPrs = useGql ? await (async () => {
          try {
            return await withTimeout(fetchPRsForRepoGQL(okRead, full, void 0), repoTimeoutMs, `prs GQL ${full}`);
          } catch {
            return await withTimeout(fetchPRsForRepo(okRead, full), repoTimeoutMs, `prs REST ${full}`);
          }
...
function writeJson(outDir, file, data) {
  fs4__namespace.default.mkdirSync(outDir, { recursive: true });
  const p = path__default.default.join(outDir, file);
  fs4__namespace.default.writeFileSync(p, JSON.stringify(data, null, 2));
}
...
var RequestHandlerHelper = class {
  ...
  makeRequest() {
    ...
    return new Promise((_resolve, _reject) => {
      ...
      const cleanupListener = new events.EventEmitter();
      const req = this.req;
...
var TweetStreamEventCombiner = class extends events.EventEmitter {
  constructor(stream) {
    super();
    this.stream = stream;
    this.stack = [];
...
var TweetStreamParser = class extends events.EventEmitter {
  constructor() {
    super(...arguments);
    this.currentMessage = "";
  }
...
var TweetStream = class extends events.EventEmitter {
...
var OAuth1Helper = class _OAuth1Helper {
  ...
  hash(base, key) {
    return crypto__namespace.createHmac("sha1", key).update(base).digest("base64");
  }
...
var OAuth2Helper = class {
  ...
  static getCodeChallengeFromVerifier(verifier) {
    return this.escapeBase64Url(crypto__namespace.createHash("sha256").update(verifier).digest("base64"));
  }
...
async function readFileIntoBuffer(file) {
  ...
  if (typeof handle === "number") {
    return new Promise((resolve, reject) => {
      fs4__namespace.readFile(handle, (err, data) => {
        if (err) {
          return reject(err);
        }
        resolve(data);
...
function getFileHandle(file) {
  if (typeof file === "string") {
    return fs4__namespace.promises.open(file, "r");
...
async function getFileSizeFromFileHandle(fileHandle) {
  if (typeof fileHandle === "number") {
    const stats = await new Promise((resolve, reject) => {
      fs4__namespace.fstat(fileHandle, (err, stats2) => {
        if (err)
          reject(err);
        resolve(stats2);
      });
...
async function readNextPartOf(file, chunkLength, bufferOffset = 0, buffer) {
  ...
  if (typeof file === "number") {
    bytesRead = await new Promise((resolve, reject) => {
      fs4__namespace.read(file, buffer, 0, chunkLength, bufferOffset, (err, nread) => {
        if (err)
          reject(err);
        resolve(nread);
      });
...
async function main() {
  ...
  let repos = [];
  try {
    const plan = JSON.parse(fs4__namespace.default.readFileSync("plan.json", "utf8"));
...
//# sourceMappingURL=sync-shard.cjs.map
//# sourceMappingURL=sync-shard.cjs.map