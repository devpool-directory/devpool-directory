...
// src/artifacts/merge.ts
function mergeIssues(chunks) {
...
function mergeMirrorState(chunks) {
  return Object.assign({}, ...chunks);
}
function computeStatistics(issues, mirror) {
...
function writeJson(outDir, file, data) {
  fs2__default.default.mkdirSync(outDir, { recursive: true });
  const p = path2__default.default.join(outDir, file);
  fs2__default.default.writeFileSync(p, JSON.stringify(data, null, 2));
}
...
function requestLog(octokit) {
  octokit.hook.wrap("request", (request2, options) => {
    ...
    return request2(options).then((response) => {
      ...
      octokit.log.info(
        `${requestOptions.method} ${path3} - ${error.status} in ${Date.now() - start}ms`
      );
      throw error;
    });
  });
...
async function ensureBranch(octokit, owner, repo, branch) {
  try {
    ...
    const { data: main2 } = await octokit.repos.get({ owner, repo });
    const defaultBranch = main2.default_branch;
    let baseSha;
    try {
...
async function main() {
  const shardsDir = path2__default.default.join(process.cwd(), "shards");
  const shardDirs = fs2__default.default.existsSync(shardsDir) ? fs2__default.default.readdirSync(shardsDir) : [];
  const issueChunks = [];
  const prChunks = [];
  const mirrorChunks = [];
  const ownerChunks = [];
  ...
  let tweetsCreated = 0;
  let tweetsDeleted = 0;
  const shardIdSet = /* @__PURE__ */ new Set();
  for (const d of shardDirs) {
    const dir = path2__default.default.join(shardsDir, d);
    if (!fs2__default.default.lstatSync(dir).isDirectory()) continue;
    for (const f of fs2__default.default.readdirSync(dir)) {
      if (f.endsWith("-issues.json")) issueChunks.push(JSON.parse(fs2__default.default.readFileSync(path2__default.default.join(dir, f), "utf8")));
      if (f.endsWith("-prs.json")) prChunks.push(JSON.parse(fs2__default.default.readFileSync(path2__default.default.join(dir, f), "utf8")));
      if (f.endsWith("-mirror-state.json")) mirrorChunks.push(JSON.parse(fs2__default.default.readFileSync(path2__default.default.join(dir, f), "utf8")));
      if (f.endsWith("-owners.json")) ownerChunks.push(JSON.parse(fs2__default.default.readFileSync(path2__default.default.join(dir, f), "utf8")));
      if (f.endsWith("-sync-meta.json")) syncChunks.push(JSON.parse(fs2__default.default.readFileSync(path2__default.default.join(dir, f), "utf8")));
      if (f.endsWith("-twitter-delta.json")) twitterDeltas.push(JSON.parse(fs2__default.default.readFileSync(path2__default.default.join(dir, f), "utf8")));
