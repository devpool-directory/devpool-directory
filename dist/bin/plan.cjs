...
// src/discovery.ts
async function discoverRepos(octokit, cfg) {
...
async function tryLoadJsonFromStorage(octokit, owner, repo, path3, ref) {
  try {
    const { data } = await octokit.repos.getContent({ owner, repo, path: path3, ref });
...
function greedyBalanceShards(repos, weights, K) {
  const items = repos.map((r) => ({ repo: r, w: weights.get(r) ?? 1 })).sort((a, b) => b.w - a.w);
  const shards = Array.from({ length: K }, (_, i) => ({ shard_id: i, repos: [], sum: 0 }));
  ...
  return shards.map(({ shard_id, repos: repos2 }) => ({ shard_id, repos: repos2 }));
}
async function main() {
  ...
  for (const r of repos) weights.set(r, 1);
  if (ctx) {
    const branch = cfg.data_branch || "__STORAGE__";
    const openPriced = await tryLoadJsonFromStorage(octokit, ctx.owner, ctx.repo, "partner-open-issues.json", branch);
