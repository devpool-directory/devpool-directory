...
function writeJson(outDir, file, data) {
  ...
  const p = path2.join(outDir, file);
...
async function main() {
  const shardsDir = path2.join(process.cwd(), "shards");
  const shardDirs = fs2.existsSync(shardsDir) ? fs2.readdirSync(shardsDir) : [];
  const issueChunks = [];
  const prChunks = [];
  const mirrorChunks = [];
  const ownerChunks = [];
  ...
  for (const d of shardDirs) {
    const dir = path2.join(shardsDir, d);
    if (!fs2.lstatSync(dir).isDirectory()) continue;
    for (const f of fs2.readdirSync(dir)) {
      if (f.endsWith("-issues.json")) issueChunks.push(JSON.parse(fs2.readFileSync(path2.join(dir, f), "utf8")));
      if (f.endsWith("-prs.json")) prChunks.push(JSON.parse(fs2.readFileSync(path2.join(dir, f), "utf8")));
      if (f.endsWith("-mirror-state.json")) mirrorChunks.push(JSON.parse(fs2.readFileSync(path2.join(dir, f), "utf8")));
      if (f.endsWith("-owners.json")) ownerChunks.push(JSON.parse(fs2.readFileSync(path2.join(dir, f), "utf8")));
      if (f.endsWith("-sync-meta.json")) syncChunks.push(JSON.parse(fs2.readFileSync(path2.join(dir, f), "utf8")));
      if (f.endsWith("-twitter-delta.json")) twitterDeltas.push(JSON.parse(fs2.readFileSync(path2.join(dir, f), "utf8")));
  ...
  const outDir = path2.join(process.cwd(), "out-agg");
