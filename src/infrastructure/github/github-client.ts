...
export class GitHubClient {
  ...
  async checkRateLimit(): Promise<void> {
    ...
    if (this.rateLimitRemaining < buffer) {
      ...
      if (resetTime > now) {
        ...
        throw new RateLimitError(`Rate limit buffer reached. Reset at ${resetTime.toISOString()}`, Math.ceil(waitTime / 1000));
      }
    }
  }

  async request<T = any>(route: string, parameters?: any): Promise<T> {
    try {
  ...
  async paginate<T = any>(route: string, parameters?: any): Promise<T[]> {
    try {
      await this.checkRateLimit();
  ...
  private handleError(error: any): never {
    if (error.status === 401) {
      throw new ExternalServiceError("GitHub", "Authentication failed. Please check your GitHub token.", 401);
    }

    if (error.status === 403) {
      throw new ExternalServiceError("GitHub", "Access forbidden. You may not have permission to access this resource.", 403);
    }

    if (error.status === 404) {
      throw new ExternalServiceError("GitHub", "Resource not found.", 404);
    }

    if (error.status === 422) {
      throw new ExternalServiceError("GitHub", "Validation failed.", 422, { errors: error.errors });
    }
    ...
    throw new ExternalServiceError("GitHub", error.message || "An unexpected error occurred", error.status, { originalError: error });
  }
