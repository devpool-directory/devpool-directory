...
export class GitHubIssueRepository implements IssueRepository {
  constructor(@inject(TYPES.GitHubClient) private githubClient: GitHubClient) {}

  async findById(owner: string, repo: string, issueNumber: number): Promise<Issue | null> {
    try {
      const response = await this.githubClient.request("GET /repos/{owner}/{repo}/issues/{issue_number}", { owner, repo, issue_number: issueNumber });
  ...
  async findAll(owner: string, repo: string, filter?: IssueFilter): Promise<Issue[]> {
    ...
    const issues = await this.githubClient.paginate("GET /repos/{owner}/{repo}/issues", parameters);

    return issues.filter((issue: any) => !issue.pull_request).map((issue: any) => this.mapToIssue(issue, owner, repo));
  }
  ...
  async findByOrganization(organization: string, filter?: IssueFilter): Promise<Issue[]> {
    ...
    const issues = await this.githubClient.paginate("GET /orgs/{org}/issues", parameters);
  ...
  async create(owner: string, repo: string, issue: Partial<Issue>): Promise<Issue> {
    const response = await this.githubClient.request("POST /repos/{owner}/{repo}/issues", {
      owner,
      repo,
      title: issue.title,
      body: issue.body,
  ...
  async update(owner: string, repo: string, issueNumber: number, updates: Partial<Issue>): Promise<Issue> {
    const response = await this.githubClient.request("PATCH /repos/{owner}/{repo}/issues/{issue_number}", {
      owner,
      repo,
  ...
  async addLabel(owner: string, repo: string, issueNumber: number, label: Label): Promise<void> {
    await this.githubClient.request("POST /repos/{owner}/{repo}/issues/{issue_number}/labels", {
      owner,
      repo,
      issue_number: issueNumber,
      labels: [label.name],
  ...
  async removeLabel(owner: string, repo: string, issueNumber: number, labelName: string): Promise<void> {
    await this.githubClient.request("DELETE /repos/{owner}/{repo}/issues/{issue_number}/labels/{name}", {
      owner,
      repo,
      issue_number: issueNumber,
      name: labelName,
  ...
  async setLabels(owner: string, repo: string, issueNumber: number, labels: Label[]): Promise<void> {
    await this.githubClient.request("PUT /repos/{owner}/{repo}/issues/{issue_number}/labels", {
      owner,
      repo,
      issue_number: issueNumber,
  ...
  async assign(owner: string, repo: string, issueNumber: number, assignees: string[]): Promise<void> {
    await this.githubClient.request("POST /repos/{owner}/{repo}/issues/{issue_number}/assignees", {
      owner,
      repo,
      issue_number: issueNumber,
      assignees,
  ...
  async unassign(owner: string, repo: string, issueNumber: number, assignees: string[]): Promise<void> {
    await this.githubClient.request("DELETE /repos/{owner}/{repo}/issues/{issue_number}/assignees", {
      owner,
      repo,
      issue_number: issueNumber,
      assignees,
  ...
  async addComment(owner: string, repo: string, issueNumber: number, comment: string): Promise<void> {
    await this.githubClient.request("POST /repos/{owner}/{repo}/issues/{issue_number}/comments", {
      owner,
      repo,
      issue_number: issueNumber,
      body: comment,
  ...
  async search(query: string): Promise<Issue[]> {
    const response = await this.githubClient.request("GET /search/issues", {
      q: query,
      sort: "created",
      order: "desc",
      per_page: 100,
  ...
  private mapToIssue(githubIssue: any, owner: string, repo: string): Issue {
    const labels = (githubIssue.labels || []).map((label: any) => Label.fromGitHub(label));

    const priceLabel = labels.find((l: Label) => l.isPriceLabel());
    const price = priceLabel ? IssuePrice.fromLabel(priceLabel.name) : IssuePrice.zero();
