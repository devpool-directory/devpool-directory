...
export class GitHubOrganizationRepository implements OrganizationRepository {
  ...
  async findRepositories(login: string, options?: {
    ...
    try {
      ...
      return data.map(repo => new Repository({
        id: repo.id,
        nodeId: repo.node_id,
        name: repo.name,
  ...
  async findMembers(login: string, options?: {
  ...
  async getOrganizationStats(login: string): Promise<{
    ...
    try {
      ...
      const [members, teams] = await Promise.all([
        this.findMembers(login).catch(() => []),
        this.findTeams(login).catch(() => [])
      ]);
