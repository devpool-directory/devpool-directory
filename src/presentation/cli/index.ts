...
program
  ...
  .description('DevPool Directory - GitHub Issue Aggregator')
  .version('2.0.0');
...
program
  ...
  .action(async () => {
    try {
      ...
      // This mimics the original main.ts behavior
      const orchestrationService = container.get<any>(TYPES.OrchestrationService);
      
      if (orchestrationService) {
        await orchestrationService.runFullSync();
      } else {
