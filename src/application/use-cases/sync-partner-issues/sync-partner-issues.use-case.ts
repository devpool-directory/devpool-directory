...
import { SyncPartnerIssuesRequest, SyncPartnerIssuesResponse, SyncedIssue, SyncError } from "./sync-partner-issues.dto";
...
export class SyncPartnerIssuesUseCase {
  ...
  async execute(request: SyncPartnerIssuesRequest): Promise<SyncPartnerIssuesResponse> {
    ...
    try {
      ...
      const existingIssues = await this.issueRepository.findAll(request.targetOwner, request.targetRepo, { state: { value: "all" } as any });

      for (const issue of partnerIssues) {
        try {
          const syncResult = await this.syncIssue(issue, existingIssues, request);
  ...
  private async fetchPartnerIssues(request: SyncPartnerIssuesRequest): Promise<Issue[]> {
    ...
    if (request.partnerRepositories && request.partnerRepositories.length > 0) {
      result = await this.issueAggregator.aggregateFromRepositories(request.partnerRepositories, aggregationOptions);
    } else {
      result = await this.issueAggregator.aggregateFromOrganizations(request.partnerOrganizations, aggregationOptions);
    }

    return result.issues;
  }

  private async syncIssue(issue: Issue, existingIssues: Issue[], request: SyncPartnerIssuesRequest): Promise<SyncedIssue> {
    const duplicates = this.duplicateDetector.detectDuplicates(issue, existingIssues, { threshold: 0.9 });
    ...
    if (duplicates.length > 0) {
      ...
      if (this.shouldUpdateIssue(issue, existingIssue)) {
        if (!request.dryRun) {
          await this.updateTargetIssue(request.targetOwner, request.targetRepo, existingIssue.number, issue, request.syncLabels);
        }
    ...
    const createdIssue = await this.createTargetIssue(request.targetOwner, request.targetRepo, issue, request.syncLabels);
  ...
  private shouldUpdateIssue(newIssue: Issue, existingIssue: Issue): boolean {
    return (
      newIssue.updatedAt > existingIssue.updatedAt || newIssue.state.value !== existingIssue.state.value || newIssue.price.value !== existingIssue.price.value
    );
  }

  private async createTargetIssue(owner: string, repo: string, sourceIssue: Issue, syncLabels?: boolean): Promise<Issue> {
    const issueData = {
  ...
  private async updateTargetIssue(owner: string, repo: string, issueNumber: number, sourceIssue: Issue, syncLabels?: boolean): Promise<Issue> {
    const updates = {
      title: this.formatTitle(sourceIssue),
      body: this.formatBody(sourceIssue),
      state: sourceIssue.state,
    ...
    const updatedIssue = await this.issueRepository.update(owner, repo, issueNumber, updates);

    if (syncLabels) {
      await this.issueRepository.setLabels(owner, repo, issueNumber, sourceIssue.labels);
    }
