...
export class DetectDuplicatesUseCase {
    ...
    try {
      ...
      for (const [sourceNodeId, groupIssues] of issuesBySource) {
        if (groupIssues.length > 1) {
          ...
          groupIssues.sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
          ...
          if (!input.dryRun) {
            for (const duplicate of duplicates) {
              try {
                await this.closeDuplicateIssue(input.owner, input.repo, duplicate.number, original.number);
                closedDuplicates++;
              } catch (error) {
      ...
      if (input.threshold) {
        const similarityGroups = await this.findSimilarIssues(issues, input.threshold);

        // Merge with existing groups
        for (const group of similarityGroups) {
          const existingGroup = duplicateGroups.find((g) => g.originalNodeId === group.originalNodeId);
  ...
  private extractSourceNodeId(labels: any[]): string | null {
    const idLabel = labels.find((label) => label.name && label.name.startsWith("id:"));
  ...
  private async findSimilarIssues(issues: any[], threshold: number): Promise<DuplicateGroup[]> {
    ...
    for (let i = 0; i < issues.length; i++) {
      ...
      for (let j = i + 1; j < issues.length; j++) {
        ...
        const similarity = this.duplicateDetector.calculateSimilarity(issue1, issue2);
  ...
  private async closeDuplicateIssue(owner: string, repo: string, duplicateNumber: number, originalNumber: number): Promise<void> {
