...
export class DuplicateDetectorService {
  ...
  detectDuplicates(issue: Issue, candidates: Issue[], options: DuplicateDetectionOptions = {}): DuplicateMatch[] {
    const {
    ...
    for (const candidate of candidates) {
      ...
      if (checkTitle) {
        const titleSimilarity = fuzzyMatch ? this.calculateStringSimilarity(issue.title, candidate.title) : issue.title === candidate.title ? 1 : 0;
      ...
      if (checkBody && issue.body && candidate.body) {
        const bodySimilarity = fuzzyMatch ? this.calculateStringSimilarity(issue.body, candidate.body) : issue.body === candidate.body ? 1 : 0;
      ...
      if (checkLabels) {
        const labelSimilarity = this.calculateLabelSimilarity(issue.labels, candidate.labels);
      ...
      if (checkRepository) {
        const repoMatch = issue.organizationName === candidate.organizationName && issue.repositoryName === candidate.repositoryName;
  ...
  detectCrossSRepositoryDuplicates(issue: Issue, candidates: Issue[]): DuplicateMatch[] {
