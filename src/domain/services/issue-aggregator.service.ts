...
export class IssueAggregatorService {
  ...
  async aggregateFromOrganizations(organizations: string[], options: AggregationOptions = {}): Promise<AggregationResult> {
    ...
    for (const org of organizations) {
      try {
        ...
        for (const repo of repos) {
          ...
          const issues = await this.issueRepository.findAll(repo.owner, repo.name, filter);
  ...
  async aggregateFromRepositories(repositories: Array<{ owner: string; name: string }>, options: AggregationOptions = {}): Promise<AggregationResult> {
    const allIssues: Issue[] = [];
    const repositorySet = new Set<string>();
    const organizationSet = new Set<string>();
  ...
  async findRelatedIssues(issue: Issue, limit: number = 10): Promise<Issue[]> {
    ...
    return results.filter((result) => result.id !== issue.id).slice(0, limit);
  }
