...
echo "Starting mirror label sync (exact match to partner)"
...
# Enumerate all issues (open + closed)
...
while : ; do
  ...
  echo "$resp" | jq -r '
    ...
    [$i.number, $bodyNorm] | @tsv' >> "$tmpdir/issues.tsv"
...
while IFS=$'\t' read -r NUM PURL; do
  ...
  names=$(jq -r '[.labels[]?.name] | .[]' < "$tmpdir/p.json" || true)
  if [ -z "$names" ]; then
    # Clear all labels on directory issue
    if gh api -X PUT "/repos/$OWNER/$REPO/issues/$NUM/labels" -f labels[]= >/dev/null 2>&1; then
      updated=$((updated+1))
    else
      failed=$((failed+1))
    fi
  ...
  # Ensure labels exist with partner color/description
  jq -r '.labels[]? | [.name, (.color//"808080"), (.description//"")] | @tsv' < "$tmpdir/p.json" | while IFS=$'\t' read -r lname lcolor ldesc; do
    ensure_label "$lname" "$lcolor" "$ldesc"
  done
  # Build -f labels[]= args
  args=( )
  while IFS= read -r n; do args+=( -f "labels[]=$n" ); done <<<"$names"
  if gh api -X PUT "/repos/$OWNER/$REPO/issues/$NUM/labels" "${args[@]}" >/dev/null 2>&1; then
    updated=$((updated+1))
  else
    failed=$((failed+1))
  fi
